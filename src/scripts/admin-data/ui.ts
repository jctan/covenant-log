...
export const createAdminDataUi = ({
  ...
  const setErrors = (
    ...
    errorTitleEl.textContent = options.title ?? 'Import/export incomplete';
  ...
  const resetPreview = () => {
    showPreviewEmpty({
      ...
      body: 'Preview results will appear here after you select a JSON snapshot and run a dry-run.'
    });
  };
  ...
  const renderPreview = (
    ...
    if (changedGroups.length === 0) {
      ...
      const cleanState = createResultItem(
        'The imported snapshot matches the local settings',
        'The dry-run found no groups that need writing — you can keep the current local configuration.'
      );
    ...
    if (options.state === 'diff') {
      ...
      previewBodyEl.textContent = `${changedGroups.length} group(s) pending update, ${totalChangedCount} field(s) will change`;
    }
    ...
    for (const group of changedGroups) {
      ...
      const resultItem = createResultItem(
        `${GROUP_LABELS[group]} · ${result.written ? 'Written' : 'Pending update'}`,
        GROUP_FILES[group],
        {
          marker: '#',
  ...
  const showBootstrapError = (message: string) => {
    ...
    showPreviewEmpty({
      state: 'error',
      title: 'Data Console failed to initialize',
      body: message
    });
  };
