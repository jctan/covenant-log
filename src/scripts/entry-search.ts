...
if (!root) {
  ...
  const updateStatusForMatches = (query: string, totalMatches: number, visibleMatches: number) => {
    ...
    if (visibleMatches === 0) {
      setStatus(`${prefix} (no results on this page — turn the page to keep browsing)`);
      return;
    }
    setStatus(`${prefix} (${visibleMatches} on this page — turn the page for more)`);
  };
  ...
  const setDegradedMode = () => {
    if (input) {
      input.placeholder = 'Failed to load index';
      input.disabled = true;
    ...
    showAllItems();
    syncSections(false);
    setStatus('Failed to load index — search disabled');
  };
