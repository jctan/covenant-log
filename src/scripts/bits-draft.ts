...
export const initBitsDraft = (): BitsDraftController | null => {
  ...
  const setAuthorPlaceholders = () => {
    ...
    if (authorAvatarEl) {
      authorAvatarEl.placeholder = defaultAuthorAvatar
        ...
        : 'A relative image path (e.g. author/avatar.webp); leave blank for the default avatar';
    }
  };
  ...
  const collectImages = (): ImageDraft[] | null => {
    ...
    for (const row of getImageRows()) {
      ...
      if (!widthValue || !heightValue) {
        setStatus('This image is filled in — please add its width and height.', 'error');
  ...
  const buildMarkdown = () => {
    ...
    if (authorAvatar === undefined) {
      setStatus('The author avatar only accepts a relative image path (e.g. author/avatar.webp) — no public/ prefix, no leading /, and no URL, .., ?, or #.', 'error');
  ...
  manualCopyBtn?.addEventListener('click', async () => {
    ...
    if (manualNote) {
      manualNote.textContent = copied ? 'Draft copied.' : 'Text selected for you — press ⌘C / Ctrl+C to copy.';
    }
  });
