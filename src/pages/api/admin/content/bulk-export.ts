...
const exportOneEntry = async (
  ...
  if (!isAdminContentExportableCollectionKey(entry.collection)) {
    return {
      result: createResult(entry, {
        status: 'skipped',
        errors: [`Export isn't supported for this collection yet: ${entry.collection}`],
        errorCodes: ['unsupported_collection']
      }),
      file: null
    };
  ...
  try {
    ...
    return {
      result: createResult(entry, {
        status: 'failed',
        errors: ['Failed to export the content source file — check local file permissions or the logs'],
...
const formatReportList = (title: string, results: readonly AdminContentBulkResult[]): string[] => {
  ...
  for (const result of results) {
    ...
    if (result.trashedPath) lines.push(`  - Trashed path: ${result.trashedPath}`);
