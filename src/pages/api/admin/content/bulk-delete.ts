...
const deleteOneEntry = async (entry: AdminContentBulkEntryInput): Promise<AdminContentBulkResult> => {
  ...
  if (!isAdminContentDeletableCollectionKey(entry.collection)) {
    return createResult(entry, {
      status: 'skipped',
      errors: [getAdminContentDeleteUnsupportedReason(entry.collection) ?? `Deletion isn't supported for this collection yet: ${entry.collection}`],
      errorCodes: ['unsupported_collection']
    });
  }
  ...
  try {
    ...
    return createResult(entry, {
      status: 'failed',
      errors: ['Failed to delete the content file — check local file permissions or the logs'],
