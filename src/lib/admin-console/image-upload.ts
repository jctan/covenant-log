...
const assertUploadFile = (file: File): void => {
  if (file.size <= 0) {
    throw new AdminImageUploadError('The image file is empty — choose it again');
  }

  if (file.size > ADMIN_IMAGE_UPLOAD_MAX_BYTES) {
    throw new AdminImageUploadError('Image exceeds 12MB — compress it before uploading', 413);
  }

  const type = file.type.trim().toLowerCase();
  if (type && !type.startsWith('image/') && type !== 'application/octet-stream') {
    throw new AdminImageUploadError('Please choose an image file');
  }
};
...
const writeUniqueImageFile = async (
  ...
  throw new AdminImageUploadError('Could not generate a usable filename — rename the image before uploading', 409);
};
...
export const uploadAdminBitsImage = async ({
  ...
  if (!src) {
    throw new AdminImageUploadError('Could not generate a savable image path for bits.images');
  }
