...
export class AdminContentDeleteConfirmationError extends Error {
  ...
  constructor(
    ...
    super(code === 'revision-conflict'
      ? 'Detected an external update to the content file — the delete was rejected. Refresh the list and try again'
      : 'Detected a mismatch between the content file path and the one confirmed — the delete was rejected. Refresh the list and try again');
...
const getTrashDestinationPath = async (
  ...
  // Preserve the original relative path so restoring can move the file straight back under src/content.
  ...
  throw new Error('Could not generate a usable content recycle-bin path');
};
...
export const moveAdminContentEntryToTrash = async (
  ...
  if (!relativePath.startsWith(expectedPrefix)) {
    throw new Error(`Refused to move a file outside the content root: ${relativePath}`);
  }
