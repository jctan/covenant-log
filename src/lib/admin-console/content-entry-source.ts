  ...
  if (fixedPage) {
    if (normalizedEntryId !== fixedPage.entryId) {
      throw new AdminContentEntryResolutionError(
        'invalid-entry-id',
        `${collection} only supports the fixed source file: ${fixedPage.sourcePath}`
      );
    }
...
export const resolveAdminContentEntrySourcePath = (
  ...
  if (fixedPage) {
    ...
    throw new AdminContentEntryResolutionError(
      'source-not-found',
      `The fixed source file for ${collection} doesn't exist: ${fixedPage.sourcePath}`
    );
  }
  ...
  if (!resolved) {
    throw new AdminContentEntryResolutionError(
      'source-not-found',
      `No content source file found: ${collection}/${normalizedEntryId}`
    );
  }
...
export const resolveAdminContentEntryIdFromSourcePath = (
  ...
  if (relative.startsWith('../') || relative === '..' || path.isAbsolute(relative)) {
    throw new AdminContentEntryResolutionError(
      'invalid-entry-id',
      `The content source file isn't under the ${collection} collection directory: ${toAdminContentRelativeProjectPath(absoluteFilePath)}`
    );
  }
  ...
  if (fixedPage) {
    ...
    for (const filePath of candidates) {
      try {
        ...
        // Keep an empty manifest when the fixed page source doesn't exist.
      }
    }
    return files;
  }
...
export const loadAdminContentSourceState = async (
  ...
  // Derive the entryId from the actual source file path, to avoid using the public id as a disk filename.
