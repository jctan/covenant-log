  ...
  src: z
    ...
    .superRefine((value, ctx) => {
      if (!normalizeBitsImageSource(value)) {
        ctx.addIssue({
          code: 'custom',
          message: 'images[].src only allows a relative image path under public/** or a remote https:// URL — no public/ prefix, no leading /, and no http, .., ?, or #'
        });
      }
    })
...
const bitsAuthorAvatar = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (normalized === undefined) {
      ctx.addIssue({
        code: 'custom',
        message: 'author.avatar only allows a relative image path (e.g. author/avatar.webp) — no public/ prefix, no leading /, and no URL, .., ?, or #'
      });
      return;
    }
  })
