...
const LEAVE_CONFIRM_MESSAGE = 'You have unsaved changes — leave this page anyway?';
...
const ARTICLE_INFO_DIALOG_TITLE = 'Post Info';
...
const storeContentListDeleteFeedback = () => {
  try {
    ...
    // The light toast after navigation only improves feedback visibility; it shouldn't affect the main delete flow.
  }
};
...
const requestPreview = async () => {
  ...
  try {
    ...
    if (!previewOutcome.responseOk || !previewOutcome.payloadOk || !previewResult) {
      ...
      previewError = payloadErrors[0] ?? 'Failed to generate the preview — check the response and console logs';
...
const deleteContentEntry = async (event: MouseEvent) => {
  ...
  const confirmed = window.confirm([
    ...
    ...(isDirty ? ['', 'Unsaved changes will not be written to the file; deleting moves the current source file.'] : []),
    '',
...
$effect(() => {
  return createEditorPageLifecycle({
    ...
    navigationGuard: {
      ...
      onBlocked: () => {
        setStatus('warn', 'Save or revert first');
      }
    },
    articleInfoTrigger: {
