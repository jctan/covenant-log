...
const LEAVE_CONFIRM_MESSAGE = "This content hasn't been saved yet — leave this page anyway?";
...
const openGalleryInsert = () => {
  setStatus('warn', "The body doesn't support image galleries yet");
};
  ...
  try {
    ...
    if (!saveOutcome.responseOk || !saveOutcome.payloadOk) {
      ...
      if (saveOutcome.status === 409 && applyLatestBaseline(
        ...
        errors = [
          ...nextErrors,
          "Loaded the latest version from disk as the conflict baseline; your current edits are preserved. Review it, then save again, or use \u201cRevert changes\u201d to load the disk version."
        ];
        setStatus('warn', 'Detected an external update — your draft has been kept');
        return;
      }

      errors = nextErrors;
      setStatus(saveOutcome.status === 409 ? 'warn' : 'error', saveOutcome.status === 409 ? 'Detected an external update' : 'Write failed');
      return;
    }

    const result = saveOutcome.result;
    if (!result) {
      errors = ['Response body is missing the result field — check the dev logs'];
    ...
    setStatus(result.changed ? 'ok' : 'ready', result.changed ? 'Content saved' : 'No changes to save');
  } catch {
...
const requestPreview = async (sourceSnapshot: string) => {
  ...
  try {
    ...
    if (!previewOutcome.responseOk || !previewOutcome.payloadOk || !previewResult) {
      previewError = previewOutcome.errors[0] ?? 'Failed to generate the preview — check the response and console logs';
      return;
    }
...
const storeContentListDeleteFeedback = () => {
  try {
    ...
    // The list feedback after deletion only improves the return experience; it doesn't affect the main delete flow.
  }
};
...
const deleteContentEntry = async (event: MouseEvent) => {
  ...
  const confirmed = window.confirm([
    ...
    ...(dirty ? ['', 'Unsaved changes will not be written to the file; deleting moves the current source file.'] : []),
    '',
  ...
  busy = true;
  clearWriteFeedback();
  setStatus('loading', 'Moving to the recycle bin…');
  ...
  try {
    ...
    if (!result || !result.deleted || !result.trashedPath) {
      errors = ['Unexpected delete response — check the dev logs'];
      issues = [];
...
$effect(() => {
  return createEditorPageLifecycle({
    ...
    navigationGuard: {
      ...
      onBlocked: () => {
        setStatus('warn', imageUploadPending ? 'Please wait for the image upload to finish' : 'Save or revert first');
      }
    },
    articleInfoTrigger: {
...
>
  ...
  <EditorWorkspace
    ...
    outlineListTabLabel="Bits List"
    outlineListEmptyText="No bits yet"
    outlinePanelLabel="Bits auxiliary outline"
  ...
  <ArticleInfoDialog
    ...
    fieldsAriaLabel="Title, description, and author"
