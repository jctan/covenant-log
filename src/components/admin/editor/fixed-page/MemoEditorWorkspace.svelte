...
const requestContentWrite = async () => {
  ...
  try {
    ...
    if (!saveOutcome.responseOk || !saveOutcome.payloadOk) {
      ...
      errors = nextErrors;
      setStatus(saveOutcome.status === 409 ? 'warn' : 'error', saveOutcome.status === 409 ? 'Detected an external update' : 'Save failed');
      return;
    }
    ...
    setStatus(result.changed ? 'ok' : 'ready', result.changed ? FIXED_PAGE_EDITOR_COPY.saved : 'No changes to save');
  } catch {
