...
const updateCreateEntryId = (value: string) => {
  ...
  setStatus('ready', 'Source filename updated', { autoClear: true });
};
...
const openEditor = async (collection: ContentInfoCollection, entryId: string, trigger: HTMLElement) => {
  ...
  try {
    ...
    if (!response.ok || !isPayloadOk(payload) || !entryPayload) {
      ...
      open = false;
      frontmatter = null;
      setStatus('error', payloadErrors.length > 0 ? 'Failed to load' : 'Failed to load — you can handle it from the edit page');
      return;
    }
    ...
    open = false;
    frontmatter = null;
    setStatus('error', 'Failed to load — please try again later');
  } finally {
...
const saveEditor = async () => {
  ...
  try {
    ...
    if (!saveOutcome) {
      errors = [];
      setStatus('error', 'Unexpected save response — check the dev logs');
      return;
    }
    ...
    if (!saveOutcome.responseOk || !saveOutcome.payloadOk) {
      ...
      const fallbackText = saveOutcome.status === 409 ? 'Detected an external update' : 'Save failed — check the current form and disk state';
      setStatus(state, payloadErrors.length > 0 ? (saveOutcome.status === 409 ? 'Detected an external update' : 'Save failed') : fallbackText);
      return;
    }
    ...
    errors = [];
    setStatus('error', 'Save request failed — please try again later');
  } finally {
    busy = false;
  }
};
...
const saveCreate = async () => {
  ...
  try {
    ...
    if (!outcome.responseOk || !outcome.payloadOk || !outcome.editHref) {
      ...
      setStatus('error', outcome.errors.length > 0 ? 'Creation failed' : 'Unexpected create response — check the dev logs');
      return;
    }

    setStatus('ok', 'Created — opening the editor');
    window.location.assign(withBase(outcome.editHref));
  } catch {
    errors = [];
    setStatus('error', 'Create request failed — please try again later');
  } finally {
    busy = false;
  }
};
...
const deleteEntry = async (trigger: HTMLElement) => {
  ...
  try {
    // The list's revision may be stale; re-read the latest file state before confirming.
    ...
    const confirmed = window.confirm([
      ...
      '',
      'The file will be moved to .trash/content/, and can be restored manually from there afterward.'
    ].join('\n'));
    ...
    if (!deleteResponse.ok || !isPayloadOk(deletePayload)) {
      ...
      const fallbackText = deleteResponse.status === 409 ? 'Detected an external update' : 'Delete failed — check the console logs';
      setStatus(state, payloadErrors.length > 0 ? (deleteResponse.status === 409 ? 'Detected an external update' : 'Delete failed') : fallbackText);
      return;
    }
    ...
    if (!result || !result.deleted || !result.trashedPath) {
      errors = [];
      setStatus('error', 'Unexpected delete response — check the dev logs');
      return;
    }
    ...
    errors = [];
    setStatus('error', 'Delete request failed — please try again later');
  } finally {
    busy = false;
  }
};
...
onMount(() => {
  ...
  if (feedback === CONTENT_LIST_ACTION_FEEDBACK_SAVED) {
    ...
    setStatus('ok', 'Moved to the recycle bin', { autoClear: true });
  }
});
...
{#if frontmatter}
  <ArticleInfoDialog
    ...
    {slugPlaceholder}
    dialogTitle={dialogMode === 'create' ? 'New Post' : selectedCollection === 'bits' ? 'Edit Info' : 'Post Info'}
    fieldsAriaLabel={selectedCollection === 'bits' ? 'Title, description, and author' : 'Essay fields'}
    {bitsDefaultAuthor}
    fieldScope={selectedCollection === 'bits' ? 'bits-summary' : 'all'}
    draftLocked={dialogMode === 'create'}
    draftLockHelp={dialogMode === 'create' ? 'Drafts by default — publish once the body is ready.' : ''}
